...
def demo_manual_registration():
    ...
    # Set Bounding corners
    # source_bounding_polygon = np.asarray([[200, 300.0,  100.0],
    #                                 [200.0, 800.0,  100.0],
    #                                 [600.0, 300.0,  100.0],
    #                                 [600.0, 800.0,  100.0],
    #                                 [600.0, 300.0,  200.0],
    ...
    file_links = open("data\\file_list.txt","r")
    content = file_links.read()
    content_list = content.split('\n')
    file_links.close()
    
    reg_results = open('data\\reg_results\\reg_results_IPCAI.csv','w')
    writer = csv.writer(reg_results)
    writer.writerow(['dataset','rad','recon_type','sum_type','fitness','inlier_rmse','iterations'])
    
    for i in range(len(content_list)):
        path = Path(content_list[i])
        source = o3d.io.read_point_cloud(content_list[i])
        if path.parts[1] == 'dataset6':
            ...
            dataset = 'Cube'
            target = o3d.io.read_point_cloud("data\LaserScans\Rubik.ply").voxel_down_sample(voxel_size=0.05)
        elif path.parts[1] == 'dataset8':
            dataset = 'Tissue'
            target = o3d.io.read_point_cloud("data\LaserScans\Chops.ply").voxel_down_sample(voxel_size=0.05)
        ...
        o3d.io.write_point_cloud("data\\temp\\temp_target.ply",target)
        ...
        o3d.io.write_point_cloud("data\\temp\\temp_source.ply",source)
        ...
        plt.plot(reg_p2p.loss_log["index"].numpy(),reg_p2p.loss_log["inlier_rmse"].numpy()) 
        plt.xlabel('Iterations')
        plt.ylabel('RMSE')
        plt.title(dataset+ ' ' + rad + ' ' + recon_type + ' ' + sum_type)
        plt.savefig('data\\reg_results\\'+dataset+rad+recon_type+sum_type+'.png')
        plt.close()  
        print('HIIII' + dataset+ ' ' + rad + ' ' + recon_type + ' ' + sum_type) 
        writer.writerow([dataset,rad,recon_type,sum_type,reg_p2p.fitness,reg_p2p.inlier_rmse,len(reg_p2p.loss_log['iteration'])])
